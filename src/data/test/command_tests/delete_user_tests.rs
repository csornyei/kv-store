...
async fn test_command_delete_user_not_allowed_after() {
    ...
    let (result, admin_session) = data.handle_command(cmd, admin_session).await.unwrap();
    assert_eq!(result, "OK".to_string());
    ...
    let (result, user_session) = data.handle_command(cmd, user_session).await.unwrap();
    assert_eq!(result, "OK".to_string());
    ...
    let cmd = Command::from_str("GET key").unwrap();
    let result = data.handle_command(cmd, user_session).await.unwrap_err();
